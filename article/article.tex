...
p_{pb}^{(r)}(t)\frac{p_{pa}^{(h)}(t_r) -e^{-(\lambda + \phi)(t_{h} - t_r)}}{U(t_{h})}&\textit{\color{gray} if $t \leq t_r \leq t_h$}\\
\frac{p_{pa}^{(h)}(t) -e^{-(\lambda + \phi)(t_{h} - t)}}{U(t_{h})}&\textit{\color{gray} if $t > t_r$}\\
\end{cases}\\
...
\begin{equation}
\begin{split}
p_{m(p,-,n)}^{(i)}(t,t_r) &\approx p_{top=p}^{(h1)}(t, t_r)U_p(t_{h1},t_{r}) p_{top=-}^{(h2)}(t_{h1}, t_i)U_p(t_{h2},t_{i}) p_n^{(i)}(t_{h2})\textit{\color{gray},}\\
