...
%For the BD-PN model we will also need to consider the case where the individual at the end of the branch is the same as at time $t$. This corresponds to a notified partner branch, as according to our model assumptions if a partner is notified, he or she is sampled (i.e. observed in our tree): Hence the individual at the beginning of the branch (internal node corresponding to transmission) should be the same as at its end (sampled partner if it finishes with a tip, or a transmission where the partner is the donor). 
%
%
...
Using $\rho=0.58$, we estimated the following parameter values: $\lambda=0.53$, $\psi=0.43$, $\psi_p=23-59$, $\rho_n=0.006-0.007$, which corresponds to $R_e = 1.2$, infectious time $\frac{1}{\psi} = 2.3$ years, and about 6--16 days between partner notification and their sampling. The results are consistent between 10 forests and shown in Table~\ref{tbl:uk}. %The results for $\rho=0.4$ and for $\rho=0.6$ were similar: $R_e$ of respectively $1.5$ and 
