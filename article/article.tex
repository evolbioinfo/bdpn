...
%\\
%q^{(p)}(i, r)  = p^{(r, o)}(t_j) e^{- \psi_n (t_i - t_r)} \textit{\color{gray} if $i$ is a \textbf{partner} tip notified by $r$ at time $t_r \leq t_i$}
%\end{cases}
%\label{eq:branch}
%\end{equation}
...
%\end{cases}\\
%l^{(p)}(i, r) &= q^{(p)}(i,r)\psi_{n}\cdot \begin{cases}
%\rho_{n} \textit{\color{gray} ~~~~~~~~if $i$ notified their most recent patner}\\
