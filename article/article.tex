...
Note that a notification will only be possible if the notifying branch is external as the notifier only notifies the most recent partner, i.e. the transmission corresponds to the notifier's parent node. %; (2) there is at least one tip in the notified subtree that can be a partner, i.e. was sampled after the notifier and whose parent's time was before notification (i.e. there was no transmission between notification and sampling). 
...
%
%\begin{equation}
%L(\mathscr{T}|\{\lambda, \psi, \rho\}) = n log(\psi\rho) + \sum\limits_{i \in \substack{internal\\nodes}} 2\lambda p^{(i0)}(t_i) p^{(i1)}(t_i)
%\end{equation}
%
...
%\begin{split}
%&\begin{cases}
%U(t) = \frac{\lambda + \psi + c_1 \frac{c_2 e^{ c_1 (t-T)} - 1}{c_2 e^{c_1 (t-T)} + 1}}{2\lambda}\\
%\\
%p^{(i)}(t) = \Big(\frac{c_2 e^{c_1 (t_i - T)} + 1}{c_2e^{c_1 (t - T)} + 1}\Big)^2e^{c_1 (t - t_i)} \\
%\\
%p^{(i,o)}(t) =  \frac{(c_2 e^{c_1 (t_i -T)} + 1) \Big(e^{c_1 (t -t_i)} e^{(\lambda + \psi)(t - t_i)}\Big)^{1/2}}{c_2e^{c_1 (t - T)} + 1} \\
%\\
%p^{(i,nh)}(t) =  e^{(\lambda + \psi)(t - t_i)}
%\end{cases},\\
